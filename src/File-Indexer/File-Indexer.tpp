...
void FileIndexer::handle_fs_event(const std::string& path, FileSystemEvent event) {
    switch (event) {
...
DynamicArray<SearchResult> FileIndexer::search(const SearchCriteria& criteria) {
    ...
    if (!criteria.terms_.empty()) {
        ...
        if (first_filter) {
            ...
            std::set_intersection(matching_paths.begin(), matching_paths.end(), name_matches.begin(), name_matches.end(), std::inserter(intersection, intersection.begin()));
    ...
    if (!criteria.size_filter.empty()) {
        ...
        if (first_filter) {
            ...
            std::set<std::string> intersection;
            std::set_intersection(matching_paths.begin(), matching_paths.end(), size_matches.begin(), size_matches.end(), std::inserter(intersection, intersection.begin()));
            matching_paths = std::move(intersection);
        }
    }
...
void FileIndexer::start_monitoring(const std::string& path) {
...
void FileIndexer::stop_monitoring() {
    fs_watcher_.stop();
}
